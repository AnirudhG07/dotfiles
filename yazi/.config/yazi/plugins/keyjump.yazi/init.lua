...
local toggle_ui = ya.sync(function(st)
	...
	File.icon = function(self, file)
		...
		local span_icon_before = file:is_hovered() and ui.Span(" " .. file:icon().text .. " ")
			or ui.Span(" " .. file:icon().text .. " "):style(icon.style)
		...
		if st.type == "global" then
			...
			elseif st.current_num > #SINGLE_KEYS then
				return st.type == nil
						and ui.Line({ span_icon_before, ui.Span(NORMAL_DOUBLE_KEYS[pos] .. " "):fg(st.opt_icon_fg) })
					or ui.Line({ ui.Span(NORMAL_DOUBLE_KEYS[pos]):fg(st.opt_icon_fg), span_icon_before })
			else
				return st.type == nil
						and ui.Line({
							span_icon_before,
							ui.Span(SINGLE_KEYS[pos] .. " "):fg(st.opt_icon_fg),
						})
