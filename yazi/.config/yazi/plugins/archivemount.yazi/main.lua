...
return {
	entry = function(_, job)
		...
		if action == "mount" then
			...
			local quoted_tmp_file = string.format("%q", tmp_file)
			local cmd_args = "archivemount " .. table.concat(files, " ") .. " " .. quoted_tmp_file
			local success, output = command_runner(ya.quote(cmd_args))
			if success then
